...
var userAgent = navigator.userAgent.toLowerCase(), initialDate = new Date(), $document = $(document),
    ...
    isTouch = "ontouchstart" in window, onloadCaptchaCallback, c3ChartsArray = [], plugins = {
...
$document.ready(function () {
        ...
        if (prevScroll != scrollY) {
            ...
            if (el.attr('data-fade') === 'true') {
                var bound = el[0].getBoundingClientRect(), offsetTop = bound.top * 2 + scrollY,
                    sceneHeight = wrapper.outerHeight(), sceneDivider = wrapper.offset().top + sceneHeight / 2.0,
    ...
    function attachFormValidator(elements) {
        ...
        var regularConstraintsMessages = [{
            ...
            newMessage: "Обов'язкове поле."
        }, {
            type: regula.Constraint.Email,
            newMessage: "Введіть дійсну електронну пошту."
        }, {type: regula.Constraint.Numeric, newMessage: "Введіть дійсний номер."}, {
    ...
    if (plugins.rdGoogleMaps.length) {
        var i;
        $.getScript("//maps.google.com/maps/api/js?key=AIzaSyA_hnNvu7poZyStmZTRzdaq3wdx4vN8msE&language=uk&sensor=false&libraries=geometry,places&v=3.7", function () {
            ...
            for (i = 0; i < plugins.rdGoogleMaps.length; i++) {
                ...
                lazyInit($googleMapItem, $.proxy(function () {
                    var $this = $(this), styles = $this.attr("data-styles");
                    $this.googleMap({
                        marker: {basic: $this.data('marker'), active: $this.data('marker-active')},
                        ...
                        onInit: function (map) {
                            ...
                            if (inputAddress.length) {
                                ...
                                var marker = new google.maps.Marker({map: map, icon: $this.data('marker-url'),});
    ...
    var interleaveEffect = {
        effect: 'slide', speed: 1200, watchSlidesProgress: true, onProgress: function (swiper, progress) {
            ...
            for (var i = 0; i < swiper.slides.length; i++) {
                $(swiper.slides[i]).find('.slide-inner').andSelf().css({transition: speed + 'ms'});
            }
    ...
    if (plugins.swiper.length) {
        ...
        for (i = 0; i < plugins.swiper.length; i++) {
            ...
            var pag = s.find(".swiper-pagination"), next = s.find(".swiper-button-next"),
                ...
                parallax = s.parents('.rd-parallax').length, swiperSlide = s.find(".swiper-slide"), autoplay = false;
    ...
    if (plugins.isotope.length) {
        ...
        for (i = 0; i < plugins.isotope.length; i++) {
            var isotopeItem = plugins.isotope[i],
                filterItems = $(isotopeItem).closest('.isotope-wrap').find('[data-isotope-filter]'), iso;
    ...
    var d3Charts = [];
    var lineChartObjectData = [1, 2, 1.3, 3, 4.6, 2, 1.8, 3, 3.3, 3.6], lineChartObject = {
        bindto: '#line-chart',
        legend: {show: false},
        color: {pattern: ['#ec931f']},
        point: {r: 4},
        ...
        data: {
            ...
            axes: {data1: 'y'},
            type: 'area',
            classes: {data1: 'stocks-rating-chart',}
        },
        ...
        axis: {
            x: {
                min: 0, max: 12.5, tick: {
                    values: fillNumbers(13), format: function (x) {
                        return ('0' + x).slice(-2);
                    }, outer: false
    ...
    var lineChartObjectData2 = [[1, 2, 1.5, 3, 2, 1.6, 2], [2.5, 3.5, 3, 4.5, 3.5, 4.3, 4.8, 5]], lineChartObject2 = {
        bindto: '#line-chart-2',
        legend: {show: false},
        color: {pattern: ['#ec931f', '#8cb1d3']},
        point: {r: 4},
        padding: {left: 30, right: 30, top: 0, bottom: 0,},
        data: {
            xs: {'data1': 'x1', 'data2': 'x2',},
            names: {data1: '2016', data2: '2017'},
            columns: [['x1', 1, 1.5, 3, 4.4, 7, 9, 12], ['x2', 1, 1.5, 3, 4.4, 7, 10, 11, 12], ['data1'].concat(lineChartObjectData2[0]), ['data2'].concat(lineChartObjectData2[1])],
            axes: {data1: 'y'}
        },
        grid: {x: {show: true}, y: {show: true}},
        labels: true,
    ...
    if (plugins.customParallax.length && !isNoviBuilder) {
        for (var k = 0; k < plugins.customParallax.length; k++) {
            var $this = $(plugins.customParallax[k]), wrapper = $('.custom-parallax-wrap'), parallax = true, speed;
            if (parallax && !isIE && !isMobile) {
                if (speed = $this.attr("data-speed")) {
                    makeParallax($this, speed, wrapper, false);
                }
